...
module Jekyll
  ...
  class AuthorIndex < Page
    ...
    def initialize(site, base, author_dir, author)
      ...
      self.read_yaml(File.join(base, '_includes/structs'), 'author-index.html')
  ...
  class Site
    ...
    def write_author_indexes
      if self.layouts.key? 'default'
        dir = self.config['author_dir'] || 'author'
        self.authors.each do |author|
          self.config['current_author'] = author
          self.write_author_index(File.join(dir, author.gsub(/_|\P{Word}/, '-').gsub(/-{2,}/, '-')), author)
        end
  ...
  module Filters
    ...
    def author_link(author)
      ...
      "<a class='author' href='/#{dir}/#{author.gsub(/_|\P{Word}/, '-').gsub(/-{2,}/, '-')}/'>#{item}</a>"
    end
